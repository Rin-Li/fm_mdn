...
def rot6d_to_rotmat_th(rot6d: torch.Tensor) -> torch.Tensor:
    """Convert rotation 6d representation (..., 6) to rotation matrices (..., 3, 3)."""
    return grahm_schmidt_th(rot6d[..., :3], rot6d[..., 3:6])


def so3_relative_angle_th(rotmat_a: torch.Tensor, rotmat_b: torch.Tensor) -> torch.Tensor:
    """Return the geodesic angle between two rotation matrices."""
    rel = rotmat_a.transpose(-1, -2) @ rotmat_b
    trace = rel[..., 0, 0] + rel[..., 1, 1] + rel[..., 2, 2]
    cos_theta = ((trace - 1.0) * 0.5).clamp(min=-1.0 + 1e-6, max=1.0 - 1e-6)
    return torch.acos(cos_theta)


def so3_geodesic_loss_th(pred_rot6d: torch.Tensor, target_rot6d: torch.Tensor) -> torch.Tensor:
    """Geodesic SO(3) loss using the current repository rot6d convention."""
    pred_rotmat = rot6d_to_rotmat_th(pred_rot6d)
    target_rotmat = rot6d_to_rotmat_th(target_rot6d)
    angle = so3_relative_angle_th(pred_rotmat, target_rotmat)
    return (angle ** 2).mean()
